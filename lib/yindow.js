...
module.exports.createYindow = function(options) {
  ...
  self.container = options.container || container
  ...
  function split(type, index, newPane) {
    ...
    if (pane.container.items.length === 1) {
      addPane(pane.container, createPane(self, _.extend({ logger: options.logger }, newPane)))
      ...
      addPane(newContainer, createPane(self, _.extend({ logger: options.logger }, newPane)))
    }
  }
  ...
  if (options.items) {
    options.items.map(function (pane) {
      addPane(container, createPane(self, _.extend({ logger: options.logger }, pane)))
    })
  } else {
    addPane(container, createPane(self, { logger: options.logger }))
  }
